...
template <class KEY_TYPE, class PAYLOAD_TYPE>
void alexolInterface<KEY_TYPE, PAYLOAD_TYPE>::bulk_load(
    std::pair<KEY_TYPE, PAYLOAD_TYPE> *key_value, size_t num, Param *param) {
  index.set_max_model_node_size(
      1 << 24); // (22,15,66M) (22,16,89M) (22,17,91M) (22,18,81M) (23,17,92M)
                // (21,17,92M) (20,17,92M)
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool alexolInterface<KEY_TYPE, PAYLOAD_TYPE>::get(KEY_TYPE key,
                                                  PAYLOAD_TYPE &val,
                                                  Param *param) {
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool alexolInterface<KEY_TYPE, PAYLOAD_TYPE>::put(KEY_TYPE key,
                                                  PAYLOAD_TYPE value,
                                                  Param *param) {
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
bool alexolInterface<KEY_TYPE, PAYLOAD_TYPE>::remove(KEY_TYPE key,
                                                     Param *param) {
  //    index.erase(key);
  //    return true;
}
...
template <class KEY_TYPE, class PAYLOAD_TYPE>
size_t alexolInterface<KEY_TYPE, PAYLOAD_TYPE>::scan(
    KEY_TYPE key_low_bound, size_t key_num,
    std::pair<KEY_TYPE, PAYLOAD_TYPE> *result, Param *param) {
  auto scan_size = index.range_scan_by_size(
